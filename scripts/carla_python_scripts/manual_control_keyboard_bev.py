...
class World(object):
    def __init__(self, carla_world, hud, args):
        ...
        self._bev_height = 80.0  # meters above the ego vehicle
    ...
    def _update_spectator(self):
        ...
        target_location = transform.location + carla.Location(z=self._bev_height)
        ...
        if self._spectator_location is None:
            ...
            self._spectator_location.z += (target_location.z - self._spectator_location.z) * smoothing

        if self._spectator_rotation is None:
            self._spectator_rotation = target_rotation
        else:
            yaw = self._lerp_angle(self._spectator_rotation.yaw, target_rotation.yaw, 0.2)
            pitch = self._lerp_angle(self._spectator_rotation.pitch, target_rotation.pitch, 0.2)
            roll = self._lerp_angle(self._spectator_rotation.roll, target_rotation.roll, 0.2)
            self._spectator_rotation = carla.Rotation(
                pitch=pitch,
                yaw=yaw,
                roll=roll)
