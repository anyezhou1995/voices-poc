...
def queue_update_acc(radar_detection_range, queue_len_per_vehicle, queue_length_w1, distance2stopbar, vehicle_current_speed, Speed_base, dist, SigState):
    ...
    if 3.1 >= vehicle_current_speed > 0:
        queue_length_w1 = np.minimum(queue_length_w1, d_front + 15)
...
def green_window_adjust(queue_discharge_speed, departure_speed, queue_length_w1, queue_length_w2, t1s, t1e, t2s, t2e):
    ...
    :return:
    """
    green_window_start1 = queue_length_w1 / queue_discharge_speed + t1s + 3  # add 2 seconds buffer time
    green_window_end1 = t1e - queue_length_w1 / departure_speed - 3

    green_window_start2 = queue_length_w2 / queue_discharge_speed + t2s + 3  # add 2 seconds buffer time
    green_window_end2 = t2e - queue_length_w2 / departure_speed - 3
...
def desire_speed4green_window_w_downstream(simsec, orginal_desire_spd, distance2stopbar, queue_length_w1,
    ...
    print(t21s)
    if distance2stopbar_next is None or t21s == '' or t21s is None:
        v1s_ = v1s
        v1e_ = v1e
        v2s_ = v2s
        v2e_ = v2e
...
def coasting_numeric_relax_sol_r(distance2stopbar, queue_coast, a_comfort, vehicle_current_speed, desired_speed, minimum_desired_speed, minimum_control_speed,
    ...
    if vehicle_current_speed > minimum_control_speed:
        ...
        if desired_speed < minimum_space_mean_spd_4:
            ...
            df_2 = pd.DataFrame(list(itertools.product(np.arange(minimum_control_speed, vehicle_current_speed, 0.5), np.arange(0, desired_speed, 0.5))), columns=['v_0', 'v_end'])
            ...
            df = pd.DataFrame(np.arange(0, desired_speed, 0.5), columns=['v_end'])
        ...
        a_max = ((vehicle_current_speed * 0.44704) ** 2)/(2 * (distance2stopbar - queue_coast) * 0.3048)
        ...
        if a_max > 4.572:
            ...
            df_2 = pd.DataFrame(np.arange(0, vehicle_current_speed, 0.5), columns=['v_0'])
            ...
            df_2 = df_2[(df_2['braking_a'] <= 4.572) &
                        (df_2['braking_a'] > 0) & (df_2['time_ahead'] >= 0)].reset_index(drop=True)

            if len(df_2) > 0 and df_2.braking_time.min() >= 1:
            ...
            df = pd.DataFrame(list(itertools.product(np.arange(desired_speed, vehicle_current_speed, 0.5), np.arange(0, desired_speed, 0.5))), columns=['v_0', 'v_end'])
...
def IntelligentDriverModel(vehicle_current_speed, orginal_desire_spd, acc_speed_base, acc_dist, minimum_gap=2, time_gap=1, a_max=1.5, b_comfort=2, acceleration_exp=4):
    ...
    if np.isnan(acc_dist):
        acc_dist = 2000
        acc_speed_base = vehicle_current_speed
...
def coasting4instant_desired_spd_toyota_v4(A, B, C, M, dist_threshold4speed_limit_1, dist_threshold4speed_limit_2,
    ...
    if sig_state == 'green' and desired_speed <= 40 and distance2stopbar <= 80:
        mode = 'IDM'
        a_instant, instant_desired_speed = IntelligentDriverModel(vehicle_current_speed, orginal_desire_spd, acc_speed_base, acc_dist)
            ...
            if desired_speed == orginal_desire_spd:
                ...
                if desired_speed < vehicle_current_speed:
                    ...
                    if mode == 'Cruising':
                        ...
                        a_instant, instant_desired_speed = IDM4LeadVehicleOrRedLight(vehicle_current_speed, minimum_desired_speed, acc_speed_base, acc_dist, distance2stopbar)

                elif vehicle_current_speed == 0:
                    if sig_state == 'red': # vehicle stop during red light
                        # instant_desired_speed = 0
                        mode = 'Stopping'
                        a_instant, instant_desired_speed = IDM4LeadVehicleOrRedLight(vehicle_current_speed,
                                                                                    minimum_desired_speed, acc_speed_base,
    ...
    if mode == 'IDM':
        ...
        if abs(-a_deceleration - current_acceleration) > 2:
            if -a_deceleration > current_acceleration:
                a_deceleration = -(current_acceleration + 2)
            else:
                a_deceleration = -(current_acceleration - 2)
...
def gen_desired_spd(example_coasting_profile, A, B, C, M, orginal_desire_spd, next_movement, vehicle_current_speed, current_acceleration, simsec, distance2stopbar,
    ...
    """
    Check all the units
    :param example_coasting_profile: Dataframe, read from local csv file.
    :param A: vehicle parameter
    :param B: vehicle parameter
    :param C: vehicle parameter
    :param M: vehicle parameter
    :param orginal_desire_spd: configuration parameter
    :param next_movement: configuration parameter
    :param vehicle_current_speed: speed, mph
    :param current_acceleration:
    :param simsec:
    :param distance2stopbar:
    :param acc_speed_base:
    :param acc_dist:
    :param num_through_lanes:
    :param upstream_flow_rate:
    :param upstream_flow_speed:
    :param SigState:
    :param t1s:
    :param t1e:
    :param t2s:
    ...
    if next_movement == 'TH':
        ...
        queue_length_w1 = (v_ac / (v_ac + orginal_desire_spd)) * (distance2stopbar - orginal_desire_spd * 1.46667 * (r1s - simsec))
        ...
        if acc_dist != 'None': # udpate based on ACC function.
